...
- **Sistema de diseño**: tokens CSS en `:root` (color, tipografía, espaciado `--s1`…`--s10`, forma, movimiento). Tipografías: `Instrument Serif` (display), `Manrope` (interfaz y texto), `JetBrains Mono` (rótulos, fechas y cifras). Acento dorado sobre papel cálido y tinta, heredado de la identidad anterior; el contraste AA está verificado en los dos temas.
